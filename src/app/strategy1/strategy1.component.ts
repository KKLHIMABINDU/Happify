...
export class Strategy1Component implements OnInit {
  ...
  actionMethod(event:any) {
    ...
    console.log(this.user.strdate1)
    if (this.user.strdate1 == null || Math.floor((this.user.strdate1 - this.latestdate)/ 1000 / 60 / 60 / 24)>=1){
      this.user.strdate1 = curdate;
      this.user.count2 = this.user.count1+1;
    }
  ...
  days(){
    ...
    var eventStartTime = new Date(this.user.strategyenrolled1);
